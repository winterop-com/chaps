...
| `user <model>` | does the account the overlay runs each enabled model as still match what its image declares (one line per enabled model) | `docker image inspect` of the exact tag the overlay pins, read from the local daemon only - no network, so this line is answered under `--offline` too. Warns when the two have drifted apart, which is what a `user:` written by an older `chaps` (or by hand) looks like: an image that runs as root under `user: 1000:1000` starts fine and then fails on its own binaries. `chaps models enable <id>` reads the account off the image again, and `chaps update` does it for a model it moves to a new tag. Skipped when the image is not pulled here, when there is no docker CLI to ask through, and when the image runs as an account name only the image itself can turn into numbers. See [Data directories and users](./models.md#data-directories-and-users). |

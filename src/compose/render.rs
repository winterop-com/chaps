...
/// tag, byte for byte but for its line endings, behind the same two-line
/// header the vendored copy carries; without it the copy compiled into the
/// binary is filled in. Both renderings are a pure function of the project
/// state, which is what lets `chaps sync` treat `compose.yml` as an artifact.
...
pub fn render_chaps_overlay(api_port: u16, project_name: Option<&str>) -> String {
    format!(
        "# {CHAPS_COMPOSE} - rendered by chaps sync from .chaps/project.yaml. \
         ...
         \x20   environment:\n\
         \x20     # chap-core checks this shared secret on every service registration, and\n\
         \x20     # upstream's compose.ghcr.yml passes only {API_TOKEN_ENV_VAR} into the\n\
         \x20     # container - so without this line a protected deployment answers every\n\
         \x20     # model's registration with HTTP 401. Empty when .env sets none, which\n\
         \x20     # chap-core reads as no key at all.\n\
         \x20     {REGISTRATION_KEY_ENV_VAR}: ${{{REGISTRATION_KEY_ENV_VAR}:-}}\n",
        project_name_block(project_name)
    )
}

/// The top-level `name:` key, with the note that says why it is there.
///
...
pub(crate) fn project_name_block(project_name: Option<&str>) -> String {
    match project_name {
        Some(name) => format!(
            "# The compose project name: the prefix every container and volume of this\n\
             # deployment carries. Without it compose would use the directory name, which\n\
             # another deployment can share.\n\
             name: {name}\n"
        ),
        None => String::new(),
    }
...
pub fn render_umbrella(files: &[String], project_name: Option<&str>) -> String {
    ...
    out.push_str(
        "# compose.marketplace.yml - generated by chaps; rendered from .chaps/models.yaml by `chaps sync`.\n\
         ...
         # Requires Docker Compose v2.20+ for the `include` directive, and v2.24.4+\n\
         # for the `!override` tag in compose.chaps.yml.\n",
    );
...
mod tests {
    ...
    fn the_chaps_overlay_names_the_compose_project() {
        ...
        // The key comes with the reason it is there.
        assert!(text.contains("# The compose project name:"), "{text}");
    ...
    fn the_chaps_overlay_hands_chap_core_the_registration_key() {
        ...
        assert!(
            text.contains("# chap-core checks this shared secret"),
            "{text}"
        );
    }
